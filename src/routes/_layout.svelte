...
<header style="z-index: 2;" class="fixed top-0 w-full bg-white">
	...
	<div class="w-full flex items-center px-4 py-2">
		...
		<div class="flex items-center px-4 py-2 relative">
			<div class="w-8 h-8 rounded-md flex justify-center items-center opacity-70" style="{ services.find((x) => x.id == current).icon.background }">
		...
		<!-- Divider -->
		<div style="width: 0.05rem;" class="h-8 bg-gray-800 opacity-40 rounded-md mx-2"></div>

		<div style="overflow: hidden; overflow-y: auto;" class="flex-grow flex items-center">
		...
		<!-- Account -->
		<div class="flex items-center px-4 h-12 relative bg-gray-100 rounded-md">
